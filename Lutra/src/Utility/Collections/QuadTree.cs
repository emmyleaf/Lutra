...
public class QuadTree<T>
{
    ...
    public void DebugDrawQuadsIMGUI(Color color)
    {
        foreach (var quad in root.Quads)
        {
            ImGuiHelper.DrawRectangle(quad.MinX, quad.MinY, quad.MaxX - quad.MinX, quad.MaxY - quad.MinY, color);
        }
        ...
        foreach (var branch in root.Branches)
        {
            if (branch != null)
            {
                DebugDrawBranchIMGUI(branch, color);
            }
        }
    }

    public void DebugDrawLeavesIMGUI(Color color)
    {
        foreach (var leaf in GetLeaves())
        {
            ImGuiHelper.DrawRectangle(leaf.Quad.MinX, leaf.Quad.MinY, leaf.Quad.MaxX - leaf.Quad.MinX, leaf.Quad.MaxY - leaf.Quad.MinY, color);
        }
    }
    ...
    private static void DebugDrawBranchIMGUI(Branch branch, Color color)
    {
        ...
        foreach (var subbranch in branch.Branches)
        {
            if (subbranch != null)
            {
                DebugDrawBranchIMGUI(subbranch, color);
            }
        }
    }
