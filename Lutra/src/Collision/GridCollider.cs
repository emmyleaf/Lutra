...
namespace Lutra.Collision
{
    ...
    public class GridCollider : Collider
    {
        ...
        public override void DebugRenderIMGUI(Color? color = null)
        {
            ...
            float viewLeft = camera.X - TileWidth;
            float viewRight = camera.X + camera.Width;
            float viewTop = camera.Y - TileHeight;
            float viewBottom = camera.Y + camera.Height;
            ...
            for (int i = 0; i < TileColumns; i++)
            {
                for (int j = 0; j < TileRows; j++)
                {
                    ...
                    if (Left + i * TileWidth > viewLeft && Left + i * TileWidth < viewRight && Top + j * TileHeight > viewTop && Top + j * TileHeight < viewBottom)
                    {
                        if (GetTile(i, j))
                        {
                            ImGuiHelper.DrawRectangle(Left + i * TileWidth + 1, Top + j * TileHeight + 1, TileWidth - 2, TileHeight - 2, color.Value);
                        }
                    }
                }
            }
